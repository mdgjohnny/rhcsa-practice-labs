...
check \'run_ssh "$NODE1_IP" "getfacl /groups/sales 2>/dev/null | grep -q "user:alex:rwx""\' \
    "User alex has full ACL on /groups/sales" \
    "User alex does not have ACL on /groups/sales"

check \'run_ssh "$NODE1_IP" "getfacl /groups/account 2>/dev/null | grep -q "user:alex:rwx""\' \
