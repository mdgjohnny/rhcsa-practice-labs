...
check \'run_ssh "$NODE1_IP" "rpm -q httpd &>/dev/null"\' \
...
check \'run_ssh "$NODE1_IP" "grep -rq "Listen.*8080\|:8080" /etc/httpd/conf/ /etc/httpd/conf.d/ 2>/dev/null"\' \
...
check \'run_ssh "$NODE1_IP" "semanage port -l 2>/dev/null | grep http | grep -q 8080"\' \
