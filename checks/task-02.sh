#!/usr/bin/env bash
# Task: Add a secondary IP address 10.0.99.2/24 to your primary network interface. The IP must be active now AND persist across reboots.
# Title: Configure Secondary IP Address
# Category: networking
# Target: node2

check 'ip addr show | grep -q "10.0.99.2"' \
    "Secondary IP 10.0.99.2 is active" \
    "Secondary IP 10.0.99.2 is not active (did you reload the connection?)"
