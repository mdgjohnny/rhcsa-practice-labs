...
IPV4_ADDR_LOCAL=$(run_ssh "$NODE2_IP" "hostname -I | awk '{print \$1}'" 2>/dev/null)
IPV4_GW_LOCAL=$(run_ssh "$NODE2_IP" "ip route show default | awk '/default/ {print \$3}'" 2>/dev/null)
IPV4_DNS_LOCAL=$(run_ssh "$NODE2_IP" "awk '/nameserver/{print \$2; exit}' /etc/resolv.conf" 2>/dev/null)
