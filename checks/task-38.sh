...
check 'ssh $SSH_OPTS "$NODE2_IP" "[[ -f /var/tmp/lnfile1 ]]" 2>/dev/null' \
    "File /var/tmp/lnfile1 exists on node2" \
    "File /var/tmp/lnfile1 does not exist on node2"

check 'ssh $SSH_OPTS "$NODE2_IP" "[[ -f /var/tmp/hard1 ]]" 2>/dev/null' \
    "Hard link hard1 exists on node2" \
    "Hard link hard1 does not exist"

check 'ssh $SSH_OPTS "$NODE2_IP" "[[ -f /var/tmp/hard2 ]]" 2>/dev/null' \
    "Hard link hard2 exists on node2" \
    "Hard link hard2 does not exist"

check 'ssh $SSH_OPTS "$NODE2_IP" "[[ -f /var/tmp/hard3 ]]" 2>/dev/null' \
    "Hard link hard3 exists on node2" \
    "Hard link hard3 does not exist"

# Check they share the same inode
check 'ssh $SSH_OPTS "$NODE2_IP" "[[ \$(stat -c %i /var/tmp/lnfile1) == \$(stat -c %i /var/tmp/hard1) ]]" 2>/dev/null' \
