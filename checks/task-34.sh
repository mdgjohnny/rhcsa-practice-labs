...
check \'run_ssh "$NODE1_IP" "mount | grep -q "/mnt/stdfs1.*ext4""\' \
    "/mnt/stdfs1 is mounted with ext4" \
    "/mnt/stdfs1 is not mounted with ext4"

check \'run_ssh "$NODE1_IP" "blkid | grep -q "LABEL=\"stdlabel\"""\' \
    "Filesystem with label stdlabel exists" \
    "No filesystem with label stdlabel found"

check \'run_ssh "$NODE1_IP" "grep -q "stdlabel\|/mnt/stdfs1" /etc/fstab"\' \
    "/mnt/stdfs1 is in /etc/fstab" \
    "/mnt/stdfs1 is not in /etc/fstab"

check \'run_ssh "$NODE1_IP" "test -f /mnt/stdfs1/stdfile1"\' \
