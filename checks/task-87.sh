...
check \'run_ssh "$NODE1_IP" "grep -q "/home" /etc/auto.master || grep -qr "/home" /etc/auto.master.d/"\' \
    "Automount entry for /home exists in auto.master" \
    "No automount entry for /home found"

# Check for wildcard configuration
check \'run_ssh "$NODE1_IP" "grep -rq "\*" /etc/auto.home /etc/auto.master.d/ 2>/dev/null || grep -q "\*.*nfs" /etc/auto.* 2>/dev/null"\' \
