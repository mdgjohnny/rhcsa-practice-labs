...
check \'run_ssh "$NODE1_IP" "firewall-cmd --zone=public --list-ports | grep -q "8400/udp""\' \
    "Port 8400/UDP is open in public zone" \
    "Port 8400/UDP is not open in public zone"

check \'run_ssh "$NODE1_IP" "firewall-cmd --permanent --zone=public --list-ports | grep -q "8400/udp""\' \
