...
check \'run_ssh "$NODE1_IP" "mount | grep -q "/repo""\' \
    "Something is mounted on /repo" \
    "Nothing is mounted on /repo"

check \'run_ssh "$NODE1_IP" "grep -q "file:///repo/BaseOS" /etc/yum.repos.d/* 2>/dev/null"\' \
    "BaseOS repository configured from /repo" \
    "BaseOS repository not configured from /repo"

check \'run_ssh "$NODE1_IP" "grep -q "file:///repo/AppStream" /etc/yum.repos.d/* 2>/dev/null"\' \
