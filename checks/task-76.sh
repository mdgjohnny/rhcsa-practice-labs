...
check 'ssh $SSH_OPTS "$NODE2_IP" "[[ -d /home/edwin/.config/systemd/user ]]"' \
    "Systemd user directory exists for edwin" \
    "Systemd user directory does not exist"

check 'ssh $SSH_OPTS "$NODE2_IP" "ls /home/edwin/.config/systemd/user/*.service 2>/dev/null | grep -qi container"' \
