...
LV_SIZE=$(ssh $SSH_OPTS "$NODE2_IP" "lvs --noheadings -o lv_size --units m vg1/lv1 2>/dev/null" | tr -d ' m')
...
check 'ssh $SSH_OPTS "$NODE2_IP" "df -m /mnt/lvfs1 | tail -1 | awk \"{print \\\$2}\" | grep -q \"^1[34]\"" 2>/dev/null' \
