...
check \'run_ssh "$NODE1_IP" "test -f /usr/testfile1"\' \
    "File /usr/testfile1 exists" \
    "File /usr/testfile1 does not exist"

check \'run_ssh "$NODE1_IP" "ls -Z /usr/testfile1 2>/dev/null | grep -q shadow_t"\' \
    "/usr/testfile1 has shadow_t context" \
    "/usr/testfile1 does not have shadow_t context"

check \'run_ssh "$NODE1_IP" "semanage fcontext -l | grep -q "/usr/testfile1.*shadow_t""\' \
