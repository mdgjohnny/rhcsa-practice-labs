...
check \'run_ssh "$NODE1_IP" "podman ps 2>/dev/null | grep -qi mariadb || docker ps 2>/dev/null | grep -qi mariadb"\' \
    "MariaDB container is running" \
    "MariaDB container is not running"

# Check if port 3206 is exposed
check \'run_ssh "$NODE1_IP" "podman ps 2>/dev/null | grep -q "3206" || docker ps 2>/dev/null | grep -q "3206" || ss -tlnp | grep -q ":3206""\' \
    "Port 3206 is exposed" \
    "Port 3206 is not exposed"

# Check if bind mount directory exists
check \'run_ssh "$NODE1_IP" "test -d /opt/mariadb"\' \
    "Directory /opt/mariadb exists" \
    "Directory /opt/mariadb does not exist"

# Check if bind mount is configured
check \'run_ssh "$NODE1_IP" "podman inspect $(podman ps -q --filter ancestor=mariadb 2>/dev/null) 2>/dev/null | grep -q "/opt/mariadb" || docker inspect $(docker ps -q --filter ancestor=mariadb 2>/dev/null) 2>/dev/null | grep -q "/opt/mariadb""\' \
