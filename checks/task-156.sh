...
# Check users exist
check \'run_ssh "$NODE1_IP" "id user555" &>/dev/null\' \
    "User user555 exists" \
    "User user555 does not exist"

check \'run_ssh "$NODE1_IP" "id user666" &>/dev/null\' \
    "User user666 exists" \
    "User user666 does not exist"

check \'run_ssh "$NODE1_IP" "id user777" &>/dev/null\' \
    "User user777 exists" \
    "User user777 does not exist"

# Check users have nologin shell
check \'run_ssh "$NODE1_IP" "getent passwd user555" | grep -q "nologin\|/bin/false"\' \
    "user555 has no login shell" \
    "user555 has a login shell"

check \'run_ssh "$NODE1_IP" "getent passwd user666" | grep -q "nologin\|/bin/false"\' \
    "user666 has no login shell" \
    "user666 has a login shell"

check \'run_ssh "$NODE1_IP" "getent passwd user777" | grep -q "nologin\|/bin/false"\' \
    "user777 has no login shell" \
    "user777 has a login shell"

# Check output file exists
check \'run_ssh "$NODE1_IP" "test -f /var/tmp/newusers"\' \
    "File /var/tmp/newusers exists" \
    "File /var/tmp/newusers does not exist"

# Check output file contains the usernames
check \'run_ssh "$NODE1_IP" "grep -q "user555" /var/tmp/newusers && grep -q "user666" /var/tmp/newusers && grep -q "user777" /var/tmp/newusers"\' \
