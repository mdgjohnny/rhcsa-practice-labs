#!/usr/bin/env bash
# Task: Determine what SELinux context type is used for configuration files in /etc. Save the type name to /root/selinux-context.txt. Then create /root/testdir and apply that same context type to it persistently.
...
check 'ls -Zd /root/testdir 2>/dev/null | grep -q "etc_t"' \
    "/root/testdir has matching SELinux context" \
    "/root/testdir does not have matching context"

check 'semanage fcontext -l | grep -q "/root/testdir"' \
    "Context is set persistently" \
    "Context may not survive reboot"
