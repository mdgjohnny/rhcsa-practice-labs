...
check \'run_ssh "$NODE1_IP" "test -d /share1"\' \
    "Directory /share1 exists on node1" \
    "Directory /share1 does not exist on node1"

check \'run_ssh "$NODE1_IP" "grep -q "/share1" /etc/exports 2>/dev/null"\' \
...
check 'run_ssh "$NODE2_IP" "mount | grep -q /share2" 2>/dev/null' \
    "/share2 is mounted on node2" \
    "/share2 is not mounted on node2"

check 'run_ssh "$NODE2_IP" "grep -q share2 /etc/fstab" 2>/dev/null' \
