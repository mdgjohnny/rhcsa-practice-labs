...
check 'ssh "$NODE2_IP" "grep -q \"^PermitRootLogin yes\" /etc/ssh/sshd_config"' \
    "Root login is permitted on node2" \
    "Root login is not permitted on node2"

check 'ssh "$NODE2_IP" "grep -q \"^Port 2022\" /etc/ssh/sshd_config"' \
    "SSH port 2022 configured on node2" \
    "SSH port 2022 not configured on node2"

check 'ssh "$NODE2_IP" "ss -tlnp | grep -q :2022"' \
