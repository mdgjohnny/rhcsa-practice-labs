...
check \'run_ssh "$NODE1_IP" "id user200" &>/dev/null\' \
    "User user200 exists" \
    "User user200 does not exist"

check \'run_ssh "$NODE1_IP" "id user300" &>/dev/null\' \
    "User user300 exists" \
    "User user300 does not exist"

check \'run_ssh "$NODE1_IP" "test -d /home1"\' \
    "Directory /home1 exists" \
    "Directory /home1 does not exist"

check \'run_ssh "$NODE1_IP" "grep -q "/home1" /etc/auto.master 2>/dev/null || grep -qr "/home1" /etc/auto.master.d/ 2>/dev/null"\' \
    "Autofs configured for /home1" \
    "Autofs not configured for /home1"

check \'run_ssh "$NODE1_IP" "systemctl is-active autofs &>/dev/null"\' \
