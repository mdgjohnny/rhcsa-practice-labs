...
check 'ssh $SSH_OPTS "$NODE2_IP" "id user80 &>/dev/null"' \
    "User user80 exists on node2" \
    "User user80 does not exist"

check 'ssh $SSH_OPTS "$NODE2_IP" "[[ -d /data01 ]]"' \
    "/data01 exists on node2" \
    "/data01 does not exist on node2"

check 'ssh $SSH_OPTS "$NODE2_IP" "loginctl show-user user80 2>/dev/null | grep -q Linger=yes"' \
