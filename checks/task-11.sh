...
check \'run_ssh "$NODE1_IP" "id alex" &>/dev/null\' \
    "User alex exists" \
    "User alex does not exist"

check \'run_ssh "$NODE1_IP" "getent group dbadmin" &>/dev/null\' \
    "Group dbadmin exists" \
    "Group dbadmin does not exist"

check \'run_ssh "$NODE1_IP" "getent group accounting" &>/dev/null\' \
...
check \'run_ssh "$NODE1_IP" "getent passwd santos" | grep -qE "nologin|/bin/false"\' \
